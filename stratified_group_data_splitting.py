...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 10:34:26 2020

ti crei un elemento StratifiedGropKFold(n_splits, random_state, shuffle).
n_splits è il numero di fold che vuoi create, poi per avere delle fold sempre uguali ogni volta che runni devi mettere shuffle=False (il seed in quel caso non serve).
Per esempio se vuoi ottenere un validation del 10%, fai n_split = 10, e prendi solo una delle 10 coppie che ti crea
Comunque se l'elemento StratifiedGroupKFold lo hai chiamato x ad esempio
fai x.split(X,y,patients) e ti restituisce le 10 coppie (Quello che ti restituisce sono degli indici, tramite i quali ricavi i datasset che vuoi)
X, immagini, y label, patients sono i pazienti rispetto a cui raggruppare

ieri ho provato a runnare questo:
    indices = list(df.index)
    new_list=list()
    for i in indices:
        if i.startswith('aug_'):
            i = i[4:]
        elif i.startswith('aug2_'): 
            i=i[5:]
        new_list.append(i)  

    from stratified_group_data_splitting import StratifiedGroupKFold
    x = StratifiedGroupKFold(n_splits=20)
    labels = df.loc[:,'Label']
    labels = list(df.loc[:,'Label'])
    results = x.split(indices,labels,groups=new_list)
    for train_idxs, test_idxs in results:
         print("TRAIN:", new_list[train_idxs])
