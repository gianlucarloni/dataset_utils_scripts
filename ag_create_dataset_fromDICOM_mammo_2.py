...
versione 2
 nuovo task di classificazione CALCIFICATION/MASS ESCLUDENDO both
...
if __name__ == "__main__":
    ...
    path_to_csv='/media/si-lab/63bc1baf-d08c-4db5-b271-e462f3f4444d/a_e_g/datasets/breast_CMMD/CMMD_clinicaldata_revision.csv'
    ...
    for patient_name in tqdm(patients_names):
        ...
        for slice_dcm in glob.glob(os.path.join(dicom_dir,patient_name+ '*.dcm')):
            ...
            if not is_one_row:
                ...
                class_label = list(class_label['abnormality'])
                class_label = class_label[0]
            else:
                class_label = df['abnormality']
    ...
    out_CC = out_CC_df[out_CC_df['Label']!='both']
    out_MLO = out_MLO_df[out_MLO_df['Label']!='both']

    out_CC.to_csv(os.path.join(png_dir,'CC', 'labels_abnormality.csv'), index=False)
    out_MLO.to_csv(os.path.join(png_dir,'MLO', 'labels_abnormality.csv'), index=False)
